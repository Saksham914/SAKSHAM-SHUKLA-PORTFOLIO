...
export default function PortfolioShowcase() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('setPortfolioTab' as any, handleSetTab)
  }, [])
  ...
  const [previewCertUrl, setPreviewCertUrl] =
    useState('')
      ...
      <AnimatePresence>
        {previewOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[999] bg-black/90 backdrop-blur-md flex flex-col items-center justify-center px-6 py-10"
          >
            <button
              onClick={() => {
              ...
              className="absolute top-6 right-6 w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition text-white"
            >
              <X size={18} />
            </button>
            ...
            <motion.img
              ...
              src={previewImage}
              className="max-w-[88vw] max-h-[75vh] rounded-3xl object-contain"
            />
            ...
            {previewCertUrl && (
              <a
                ...
                className="mt-6 px-6 py-2.5 rounded-full bg-white/15 hover:bg-white/25 border border-white/20 backdrop-blur-xl text-sm font-medium text-white flex items-center gap-2 transition"
              >
                <span>View Credential on LinkedIn</span>
                ...
                {!loading &&
                  certificates.map((item, i) => {
                    const certUrl =
                      ...
                      item.link_url ||
                      item.link ||
                      'https://www.linkedin.com/in/sakshamshukla123/details/certifications/'
                    ...
                    return (
                      <motion.div
                        ...
                        whileHover={{ y: -4 }}
                        className="group rounded-[26px] border border-white/10 bg-white/5 p-4 backdrop-blur-xl flex flex-col justify-between"
                      >
                        <div>
                          <div
                            onClick={() => {
                              setPreviewImage(item.image_url)
                              setPreviewCertUrl(certUrl)
                              setPreviewOpen(true)
                            }}
                            className="rounded-2xl overflow-hidden border border-white/10 h-56 relative cursor-pointer group/img bg-[#0d0d12] p-3 flex items-center justify-center"
                          >
                            <img
                              src={item.image_url}
                              alt={item.title}
                              className="w-full h-full object-contain group-hover/img:scale-105 transition duration-500"
                            />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition duration-300 flex items-center justify-center">
                              <span className="px-3.5 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-xs font-medium text-white flex items-center gap-1.5">
                                <Eye size={14} /> Preview
                              </span>
                            </div>
                          </div>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="mt-4 w-full py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 transition flex items-center justify-center gap-2 text-xs font-medium text-white/90 hover:text-white"
                        >
