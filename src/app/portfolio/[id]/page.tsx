...
import { defaultProjects } from '@/lib/portfolioService'
import {
  ArrowLeft,
  ChevronLeft,
  ChevronRight,
...
export default function PortfolioDetailPage() {
  ...
  const [project, setProject] = useState<any>(() => {
    const found = defaultProjects.find((p) => String(p.id) === String(id))
    return (
      found || {
        title: '',
        description: '',
  ...
  useEffect(() => {
    ...
    if (fallback) {
      setProject((prev: any) => ({
        ...
        description: prev?.description || fallback.description,
      }))
    }
        ...
        if (data) {
          setProject((prev: any) => ({
            ...
            live_url:
              ...
              fallback?.live_url ||
              (String(id) === '2'
                ? 'https://github.com/Saksham914/StudyNest'
                : ''),
            github_url:
              data.github_url ||
              prev?.github_url ||
              ...
              (String(id) === '2'
                ...
                : ''),
            technologies:
              data.technologies || prev?.technologies || fallback?.technologies || '',
            key_features:
              data.key_features || prev?.key_features || fallback?.key_features || '',
          }))
        }
      } catch (e) {
